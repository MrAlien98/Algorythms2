...
public class GameController extends Scene {
	...
	private ArrayList<ImageView> lasers; 
	...
	public void addEnemigosA(){
		inicializarEnemigosA();
		for(int i=0;i<getEnemigos().size();i++) {
			ajam.getChildren().add(getEnemigos().get(i));
			getEnemigos().get(i).setVisible(true);
		}
	...
	public void addLaser() {
		for(int i=0;i<lasers.size();i++) {
			ajam.getChildren().add(lasers.get(i));
		}
	}
	...
	public void moverEnemigosA() {
		setAnimationEA(new Timeline(new KeyFrame(Duration.millis(100), e->{
			for(int i=0;i<getEnemigos().size();i++) {
				disparoEnemigo();
				corregirEnemigos(Main.getGame().getListaEnemigosA().get(i));
				getEnemigos().get(i).setX(Main.getGame().getListaEnemigosA().get(i).getPosX());
			}
		})));
		animationE.setCycleCount(Timeline.INDEFINITE);
		animationE.play();
	...
	public void disparoEnemigo() {
		animacionBE=new Timeline(new KeyFrame(Duration.millis(30), e->{
			for(int i=0;i<getEnemigos().size();i++) {
				if(Main.getGame().getListaEnemigos().get(i) instanceof EnemigoA) {
					System.out.println("yeet");
					EnemigoA save=(EnemigoA)Main.getGame().getListaEnemigos().get(i);
	...
	public void inicializarEnemigosA() {
		int cont=0;
		while(cont<Main.getGame().getListaEnemigos().size()) {
			getEnemigos().add(new ImageView(new Image(Main.getGame().getListaEnemigosA().get(cont).getImagen(), 50, 50, true, true)));
			cont++;
		}
		posicionarEnemigosA();
	}
	
	public void inicializarLasers() {
		for(int i=0;i<lasers.size();i++) {
			lasers.get(i).setImage(new Image("/data/laser.png"));
			lasers.get(i).setRotate(180);
	...
	public void posicionarEnemigosA() {
		for(int i=0;i<getEnemigos().size();i++) {
			getEnemigos().get(i).setX(Main.getGame().getListaEnemigos().get(i).getPosX());
			getEnemigos().get(i).setY(Main.getGame().getListaEnemigos().get(i).getPosY());
		}
	...
	public Timeline getAnimationEA() {
		return animationEA;
	}

	public void setAnimationEA(Timeline animationEA) {
		this.animationEA = animationEA;
	}
